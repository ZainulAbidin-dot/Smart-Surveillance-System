...
private_key = '14ff9ebe852551516b8a62eea6e24826366968f499fb6455079935216a29b893'
...
class Consensus:
    ...
    def approve(tx_id):
        tx_hash = contract.functions.approve(tx_id).transact({'from': w3.eth.accounts[0], 'gas': 1000000})
    ...
    def execute(tx_id):
        ...
        tx_hash = contract.functions.execute(tx_id).transact({'from': w3.eth.accounts[0], 'gas': 1000000})
        receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    ...
    def consensus_algo():
        try:
            result1 = contract.functions.getLatestTransaction().call()
            a = int(result1)
            print("Function called : ", result1)
